"""
Configuration loader for dmc-design MCP Server.

Implements hybrid configuration system:
- System-level: ~/.config/claude/dmc-design.env (theme preferences)
...
class VizPlatformConfig:
    """Hybrid configuration loader for dmc-design tools"""
    ...
    def load(self) -> Dict[str, any]:
        ...
        """
        # Load system config
        system_config = Path.home() / '.config' / 'claude' / 'dmc-design.env'
        ...
        if project_dir:
            ...
            self.theme_dir_project = project_dir / '.dmc-design' / 'themes'
    ...
    def _find_project_directory(self) -> Optional[Path]:
        ...
        cwd = Path.cwd()
        if (cwd / '.git').exists() or (cwd / '.env').exists() or (cwd / '.dmc-design').exists():
