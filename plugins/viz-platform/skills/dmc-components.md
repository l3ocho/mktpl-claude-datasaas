...
The viz-platform MCP server (`component_registry.py`) discovers registry files dynamically
using `glob("dmc_*.json")` — there is no fixed filename. The server loads whichever
`dmc_*.json` files exist in `mcp-servers/viz-platform/registry/`, choosing the best
