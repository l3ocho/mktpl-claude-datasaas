...
Guides selection of the right Plotly `graph_objects` trace type for each analytical question. This skill covers the full `go.*` API — far beyond what the plotly-charts MCP `chart_create` tool supports (which is limited to line, bar, scatter, pie, heatmap, histogram, area).
