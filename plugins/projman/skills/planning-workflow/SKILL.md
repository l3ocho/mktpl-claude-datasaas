...
Execute in order:
1. **Branch detection** - See `skills/branch-security/SKILL.md`
2. **Repository org check** - See `skills/repo-validation/SKILL.md`
3. **Label taxonomy validation** - See `skills/repo-validation/SKILL.md`

**STOP if any validation fails.**

### 3. Detect Input Source

Follow `skills/input-detection/SKILL.md`:
...
Follow `skills/lessons-learned/SKILL.md`:
```python
...
Follow `skills/wiki-conventions/SKILL.md`:
...
Follow `skills/issue-conventions/SKILL.md` and `skills/task-sizing/SKILL.md` to prepare the complete specification for ALL issues. **Do NOT call `create_issue` yet.**
...
Output: A complete execution manifest per `skills/batch-execution/SKILL.md` Phase 2 format.

### 8a. Batch Execute Issue Creation

Follow `skills/batch-execution/SKILL.md` Phase 3:
...
**Only `skills/mcp-tools-reference/SKILL.md` is needed for this step.** Do NOT re-read other skill files.
...
Follow `skills/sprint-approval/SKILL.md`:
