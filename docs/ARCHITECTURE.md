...
- **Agents:**
  - **design-reviewer** — Dash component and layout accessibility audits; auto-detects dual-scheme CSS and runs color scheme integrity checks
  - **layout-builder** — Dashboard grid construction and responsiveness
  - **component-check** — DMC component validation and registry lookup; auto-detects dual-scheme CSS
  - **theme-setup** — Theme generation and customization; enforces dual-scope token completeness when dual-scheme detected
- **MCP:** viz-platform
...
- **Color scheme validation (v9.4.0):** `skills/color-scheme-validation.md` — 5 auditable rules for dual-scheme CSS integrity. Loaded conditionally when both `[data-mantine-color-scheme="dark"]` and `[data-mantine-color-scheme="light"]` selectors are detected. `/viz design-gate` includes Color Scheme Integrity as a blocking check. Single-scheme projects: zero behavioral change.
